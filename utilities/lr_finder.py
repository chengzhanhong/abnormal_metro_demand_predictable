...
def valley(lrs:list, losses:list) -> (float, tuple):
...
class LRFinder(object):
    ...
    def range_test(
        ...
        suggested_lr, loss_at_lr = valley(self.history["lr"], self.history["loss"])
        self.suggested_lr = suggested_lr
        self.loss_at_lr = loss_at_lr
        print("Learning rate search finished, suggested lr: {:.2E}".format(suggested_lr))
        if plot:
            fig = self.plot()
    ...
    def plot(
        ...
        ax.axvline(x=self.suggested_lr, color="red")
        ax.scatter(
            self.suggested_lr,
            self.loss_at_lr,
            s=75,
