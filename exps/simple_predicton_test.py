...
from models.MetroSimpler import MetroSimpler
...
parser.add_argument('--data_mask_ratio', type=float, default=0., help='the ratio of masked data in context')
...
def train(args, train_loader, val_loader):
    ...
    model = MetroSimpler(x_loc=x_loc, x_scale=x_scale, attn_mask=attn_mask, **vars(args))
    model.to(device)
    ...
    wandb.run.name = now + '_simple_wjw'
    args.name = run.name
