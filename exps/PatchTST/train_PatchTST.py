#%% Here patchTST refers to forecast the alighting and the boarding separately
import time
...
parser.add_argument('--datatype', type=str, default='ABT_auto_seq', help='one of seq, concat, PatchTST, ...')
...
parser.add_argument('--split_order', type=str, default='chronological', help='random or chronological, how to split train and test')
...
#%% Prepare data
args.dset = "seoul"
args.model = 'ABT_auto_seq_concat'
args.datatype = 'ABT_auto_seq_concat'
...
def train_MetroTransformer(args, train_loader, val_loader):
    ...
    wandb.run.name = now + f'_{args.dset}_ABT_auto_seq_concat'
    args.name = run.name
...
def evaluate(model, test_loader, device='cpu'):
    ...
    with torch.no_grad():
        for x,y in test_loader:
            ...
            y = y.to(device)
            y = y[:, -test_loader.dataset.num_target_patch:, :]
            y_hat = model.forecast(x)
            test_mae += mae_loss(y_hat[:,:,:patch_len], y[:,:,:patch_len]).item() * len_batch / len_data
            test_mse += rmse_loss(y_hat[:,:,:patch_len], y[:,:,:patch_len]).item()**2 * len_batch / len_data
